...
/**
 * 方便的转换工具。从原始转换到其他原语,加上支持日期、时间戳SQL日期,
 */
public final class ConverterUtil {
	...
	static {
		...
		Work toNoExpString = new Work() {
			@Override
			public Object convert(Object fromInstance) {   // Should eliminate possibility of 'e' (exponential) notation
				return fromInstance.toString();
			}
		};
	...
	public static java.sql.Date convertToSqlDate(Object fromInstance) {
		...
		try {
			if (fromInstance instanceof java.sql.Date) {   // Return a clone of the current date time because java.sql.Date is mutable.
				...
				Date date = DateUtil.parseDate(((String) fromInstance).trim());
				if (date == null) {
					return null;
				}
	...
	public static Date convertToDate(Object fromInstance) {
		...
		try {
			if (fromInstance instanceof String) {
				return DateUtil.parseDate(((String) fromInstance).trim());
